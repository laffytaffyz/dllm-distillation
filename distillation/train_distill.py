"""
Knowledge Distillation Training Script for Open-dLLM (Diffusion-Aware)

This script extends train_torch.py to support diffusion-aware knowledge distillation:
- Teacher model: Larger pre-trained diffusion LLM (e.g., fredzzp/open-dcoder-0.5B)
- Student model: Smaller model to be trained (can initialize from teacher via deepcopy)
- Diffusion sampling: Collects teacher logits during diffusion sampling steps
- Distillation loss: KL divergence between teacher and student logits at masked positions
...
def compute_distillation_loss(
    ...
    Supports both standard and diffusion-aware distillation.
    
    Args:
        student_logits: Student model logits [batch_size, seq_len, vocab_size]
        teacher_logits: Teacher model logits [batch_size, seq_len, vocab_size] or [num_masked, vocab_size]
        temperature: Temperature for softmax
        labels: Optional labels to mask loss (only compute on non-ignored tokens)
        mask_positions: Optional boolean mask for masked positions (for diffusion distillation)
    
    Returns:
        KL divergence loss
    """
    # Handle case where teacher_logits are already filtered to masked positions
    if teacher_logits.dim() == 2 and teacher_logits.shape[0] != student_logits.shape[0]:
        # Teacher logits are [num_masked, vocab_size]
        teacher_probs = F.softmax(teacher_logits / temperature, dim=-1)
        student_mask_logits = student_logits[mask_positions] if mask_positions is not None else student_logits.view(-1, student_logits.size(-1))
    ...
    # Standard case: both are [batch_size, seq_len, vocab_size]
    ...
    # Apply mask_positions if provided (for diffusion distillation)
    if mask_positions is not None:
        mask_float = mask_positions.float()
        kl_div = kl_div * mask_float
        num_masked = mask_float.sum()
        return kl_div.sum() / (num_masked + 1e-8) * (temperature ** 2)
