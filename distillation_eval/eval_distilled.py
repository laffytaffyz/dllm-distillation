...
def main():
    ...
    # Get script directory for finding eval.py
    script_dir = os.path.dirname(os.path.abspath(__file__))
    
    # Resolve model path (convert relative to absolute if needed)
    # Relative paths are resolved relative to current working directory (standard behavior)
    model_path = os.path.abspath(args.model_path)
    
    # Check if model path exists
    if not os.path.isdir(model_path):
        print(f"Error: Model path '{model_path}' does not exist!")
        print(f"  (Resolved from: {args.model_path})")
        sys.exit(1)
